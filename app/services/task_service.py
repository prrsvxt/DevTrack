"""Сценарии работы с задачами, кэшем и доменными исключениями."""
...
class TaskService:
    ...
    async def _invalidate_user_tasks_cache(self, user_id: int):
        # Удаляем все закэшированные списки пользователя после записи.
    ...
    async def create_task(self, task_data: TaskCreate, current_user: User) -> Task:
        # После записи делаем commit, refresh и инвалидируем кэш.
    ...
    async def get_task(self, current_user: User, task_id: int):
        # По ID задачу может читать только её владелец.
    ...
    async def list_tasks(self, current_user: User, status=None, deadline: date | None = None, limit: int = 10, offset: int = 0):
        # Ограничиваем размер страницы, чтобы клиент не запросил бесконечный список.
        limit = min(limit, 100)
        ...
        # Преобразуем ORM-объекты в ответный формат до записи в кэш.
    ...
    async def delete_task(self, task_id: int, current_user: User):
        # Удаление разделяем на проверку существования и прав доступа.
        task = await self.task_repository.get_by_id(task_id)
    ...
    async def update_task(self, current_user: User, task_id: int , task_updates: TaskUpdate):
        # Частичное обновление применяем только после проверки владельца.
        task = await self.task_repository.get_by_id(task_id)
