...
class TaskPermissionService:
    def __init__(
        ...
        team_permission_service: TeamPermissionService | None = None,
    ):
        self.session = session
        self.task_repository = task_repository or TaskRepository(self.session)
        self.team_permission_service = team_permission_service or TeamPermissionService(
            self.session
        )
    ...
    async def ensure_can_create_task(
        self, team_id: int | None, current_user: User
    ) -> None:
    ...
    async def ensure_can_view_task(self, task_id: int, current_user: User) -> Task:
        ...
        if task.team_id is not None:
            try:
                await self.team_permission_service.ensure_can_view_team(
                    team_id=task.team_id, current_user=current_user
                )
