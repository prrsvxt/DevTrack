...
from app.exceptions.team_member import TeamMemberAlreadyExistsError, TeamMemberNotFoundError, TeamMemberPermissionError
...
class TeamMemberService:
    ...
    async def add_member(self, team_id: int, target_user_id: int, role: TeamRole, current_user: User) -> TeamMember:
        ...
        await self.team_permission_service.ensure_can_manage_members(team_id=team_id, current_user=current_user)
    ...
    async def remove_member(self, team_id: int, target_user_id: int, current_user: User) -> None:
        ...
        user_to_delete = await self._get_team_member(team_id=team_id, user_id=target_user_id)
    ...
    async def change_member_role(self, team_id: int, target_user_id: int, new_role: TeamRole, current_user: User) -> TeamMember:
        ...
        user_to_change_role = await self._get_team_member(team_id=team_id, user_id=target_user_id)
