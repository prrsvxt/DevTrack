...
async def add_team_member(
    ...
    team_member_service: TeamMemberServiceDep,
    current_user: CurrentUser,
):
    team_member = await team_member_service.add_member(
        team_id=team_id,
...
async def get_team_members(
    team_id: int, team_member_service: TeamMemberServiceDep, current_user: CurrentUser
):
    team_members = await team_member_service.list_team_members(
...
@team_member_router.patch(
    "/{team_id}/members/{user_id}", response_model=TeamMemberResponse
)
async def update_team_member(
    team_id: int,
    user_id: int,
    role_data: UpdateTeamMemberRole,
    team_member_service: TeamMemberServiceDep,
    current_user: CurrentUser,
):
    updated_team_member = await team_member_service.change_member_role(
        team_id=team_id,
...
async def delete_team_member(
    ...
    await team_member_service.remove_member(
        team_id=team_id, target_user_id=user_id, current_user=current_user
    )
