...
async def create_team(
    team_data: CreateTeam, current_user: CurrentUser, team_service: TeamServiceDep
):
    team = await team_service.create_team(
        current_user=current_user, team_data=team_data
    )
    return team
...
async def get_team_by_id(
    team_id: int, current_user: CurrentUser, team_service: TeamServiceDep
):
...
async def update_team(
    team_id: int,
    team_updates: UpdateTeam,
    current_user: CurrentUser,
    team_service: TeamServiceDep,
):
    updated_team = await team_service.update_team(
        current_user=current_user, team_id=team_id, team_updates=team_updates
    )
    return updated_team
