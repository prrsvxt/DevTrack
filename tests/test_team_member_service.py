...
@pytest.mark.anyio
async def test_add_existing_member_raises_error(
    session, service, make_user, make_member
):
    ...
    service.team_member_repository.get_by_team_and_user = AsyncMock(
        side_effect=[current_member, target_member]
    )
...
async def test_add_member_success(session, service, make_user, make_member):
    ...
    service.team_member_repository.get_by_team_and_user = AsyncMock(
        side_effect=[current_member, None]
    )

    service.team_member_repository.create = AsyncMock(return_value=created_member)

    result = await service.add_member(
        team_id=10, target_user_id=2, role=TeamRole.DEVELOPER, current_user=current_user
    )

    assert result is created_member

    service.team_member_repository.create.assert_awaited_once_with(
        team_id=10, user_id=2, role=TeamRole.DEVELOPER
    )
...
async def test_add_owner_role_forbidden(session, service, make_user):
    ...
    with pytest.raises(TeamMemberPermissionError):
        await service.add_member(
            team_id=10, target_user_id=2, role=TeamRole.OWNER, current_user=current_user
        )
...
async def test_change_member_role_success(session, service, make_user, make_member):
    ...
    service.team_member_repository.get_by_team_and_user = AsyncMock(
        side_effect=[current_member, role_changing_member]
    )
    service.team_member_repository.update_role = AsyncMock(
        return_value=member_after_role_changed
    )

    result = await service.change_member_role(
        team_id=10,
        target_user_id=2,
        new_role=TeamRole.VIEWER,
        current_user=current_user,
    )

    assert result is member_after_role_changed

    service.team_member_repository.update_role.assert_awaited_once_with(
        role_changing_member, role=TeamRole.VIEWER
    )
...
@pytest.mark.anyio
async def test_change_member_role_same_role_returns_member_without_commit(
    session, service, make_user, make_member
):
...
@pytest.mark.anyio
async def test_not_possible_to_change_owner_role(
    session, service, make_user, make_member
):
